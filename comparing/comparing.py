...
def compute_phrase_metrics(annot_dict, bg_labels=("-1", -1)):
    ...
    # Calculate Shannon Entropy per phrase
    entropies = {}
    ...
    return frequencies, mean_durations, entropies
...
def evaluate_mapped_metrics(gt_dict, pred_dict, mapping, bg_labels=("-1", -1)):
    ...
    gt_freq, gt_dur, gt_ent = compute_phrase_metrics(gt_dict, bg_labels)
    pr_freq, pr_dur, pr_ent = compute_phrase_metrics(pred_dict, bg_labels)
...
def frame_error_rate(co_df, mapping):
    """
    Calculates frame error rate using the cooccurance matrix
    """
    correct_labels = 0
    for ground_truth, predicted in mapping.items():
        correct_labels += co_df.loc[ground_truth,predicted]

    n_bins = co_df.to_numpy().sum()
    false_labels = n_bins - correct_labels
    FER = false_labels / n_bins
...
def main():
    ...
    # calculate frame error rates 
    birdsong_FER, time_bins_1 = frame_error_rate(co_df_1, label_mapping_1)
    tweety_FER, time_bins_2 = frame_error_rate(co_df_2, label_mapping_2)

    print(f"Birdsong Frame Error Rate: {birdsong_FER}\n")
    print(f"TweetyBERT Frame Error Rate: {tweety_FER}\n")
    if (time_bins_1 != time_bins_2):
        print("...but the time annotations are off.")
        print(f"Birdsong: {time_bins_1}\nTweetyBERT: {time_bins_2}")

    # Compute Phrase Metrics: correlations of duration and entropy
    ...
    print("--- Weighted Pearson Correlations (GT vs Predicted) ---")
    print(f"Birdsong   - Duration Corr: {birdsong_corr_dur:.4f} | Entropy Corr: {birdsong_corr_ent:.4f}")
    print(f"TweetyBERT - Duration Corr: {tweety_corr_dur:.4f} | Entropy Corr: {tweety_corr_ent:.4f}")
    ...
    with open(f"{args.bird}_comparison_data.csv", mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(['Tweety FER', 'Birdsong FER', 'Tweety Entropy Cor', 'Birdsong Entropy Cor', 'Tweety Duration Cor', 'Birdsong Duration Cor', 'Audio Minutes'])

        raw_data = [
            tweety_FER, birdsong_FER, tweety_corr_ent, birdsong_corr_ent, 
