...
def validate_html(X, clusters, loop_specs, wav_segs, samplerate, all_intervals, output_file="output.html"):
    """
    ...
    ax2d.set_title("HDBSCAN Clustering on Path Signatures (UMAP)")
...
def plot_record_analysis(record, record_clusters, colors, save_name):
    ...
    - A row for each cluster containing: 
        1) The 3D embeddings of the whole record (gray) with cluster segments highlighted
        2) 3 example spectrogram segments for that cluster
    """
    ...
    gs2 = GridSpec(num_clusters, 4, figure=fig2)
    ...
    for i, c_id in enumerate(unique_clusters):
        ...
        # Column 0: 3D Embeddings
        ax3d = fig2.add_subplot(gs2[row_idx, 0], projection='3d')
        # Plot the entire continuous recording in translucent gray
        ax3d.plot(embeddings_3d[:, 0], embeddings_3d[:, 1], embeddings_3d[:, 2], 
                  color='gray', alpha=0.3, linewidth=1)
        
        # Find all intervals belonging to this specific cluster
        c_intervals = [inter for j, inter in enumerate(intervals) if record_clusters[j] == c_id]
        
        # Highlight the first three of this cluster's segments using the colormap
        num_segs = 0
        for start, end in c_intervals:
            if num_segs < 3:  
                traj = embeddings_3d[start:end]
                ax3d.plot(traj[:, 0], traj[:, 1], traj[:, 2], color=color, alpha=0.9, linewidth=2.5)
                num_segs = num_segs + 1
        ...
        # Columns 1-3: Example Spectrograms
        # Grab up to 3 examples
        examples = c_intervals[:3]
        for j, (start, end) in enumerate(examples):
            ax_ex = fig2.add_subplot(gs2[row_idx, j+1])
            ex_spec = spec[:, start:end]
            
            ax_ex.imshow(ex_spec, origin='lower', aspect=0.5, cmap='magma')
            ax_ex.set_yticks([])
            if j == 1:
                ax_ex.set_title(f"Cluster {c_id} Example Spectrograms", pad=10)
